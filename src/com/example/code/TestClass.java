...
public class TestClass {
	...
	public static void main(String[] args) {
		...
		String input="My name is Gopinath";
		...
		// ***********************************************Intermediate Questions of Java
		// 8 stream: *********************************************************
		...
		// :************************************************************
		// 1) Given a list of employees (with fields name, age, and department), group
		// employees by their department.
		//

		List<Employee> listofemp = Arrays.asList(new Employee("Girish", 27, "ERD"), new Employee("Namdev", 30, "ABS"),
				new Employee("Sagar", 32, "CompEngg"));
		Map<String, List<Employee>> empGroupByDept = listofemp.stream()
	...
	//	String input = "Today is an awesome day";
		...
		List<Integer> numbers2 = Arrays.asList(1, 2, 3, 4, 2, 3, 5, 6, 6);
		Set<Integer> abcd = new HashSet<>();
		Set<Integer> duplicates2 = numbers2.stream().filter(n -> !abcd.add(n)).collect(Collectors.toSet());
		System.out.println("duplicate elements are " + duplicates2);
       ...
       // Find non-duplicate elements using Stream API
       ...
       List<Integer> num = Arrays.asList(1, 2, 3, 41, 4, 5, 80);
		List<Integer> reverseListanddistinct = num.stream().distinct().sorted(Comparator.reverseOrder())
				.collect(Collectors.toList());
		Optional<Integer> secondlargestNum = reverseListanddistinct.stream().skip(2).findFirst();
